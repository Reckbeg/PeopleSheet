...
// ── PPh21 Tax Calculator (TER Method) ──────────────────────────
function buildPph21TaxCalculator(workbook: ExcelJS.Workbook, template: TemplateProduct, options?: TemplateBuildOptions) {
  ...
  // TER Category mapping (which PTKP → which TER category)
  ...
  // TER rates: [category, ptkpLabel, lowerBound, upperBound, rate]
  ...
  addRows(empTax, 5, employees.map((emp, i) => {
    ...
    return [
      ...
      // TER Category: VLOOKUP from Setup mapping table
      { formula: `VLOOKUP(E${r},Setup!$A$29:$B$36,2,FALSE)` },
      // Annual Gross
      { formula: `D${r}*12` },
      // BPJS Deduction (approx 5% of annual gross — employer portion)
      { formula: `G${r}*0.05` },
      // Taxable Base (DPP) = Annual Gross - BPJS - PTKP
      { formula: `G${r}-H${r}-VLOOKUP(E${r},Setup!$A$18:$B$25,2,FALSE)` },
      // TER Rate — simplified: use IF-based lookup from TER sheet
      // For simplicity, use a flat rate approximation based on income level
