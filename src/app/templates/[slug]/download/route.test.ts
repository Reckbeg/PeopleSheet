...
describe("template download route", () => {
  it("returns 404 for unknown template slug", async () => {
    const request = new Request("https://peoplesheet.id/templates/unknown/download");
  ...
  it("sanitizes risky spreadsheet cell input", async () => {
    const request = new Request(
      "https://peoplesheet.id/templates/attendance-tracker/download?companyName==SUM(1,1)",
    );
  ...
  it("clamps out-of-range year to accepted bounds", async () => {
    const request = new Request("https://peoplesheet.id/templates/leave-tracker/download?year=1800");
    const response = await GET(request, {
  ...
  it("sets private cache and attachment filename headers", async () => {
    const request = new Request("https://peoplesheet.id/templates/attendance-tracker/download");
    const response = await GET(request, {
      params: Promise.resolve({ slug: "attendance-tracker" }),
    });
